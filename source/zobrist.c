...
/* 
 * This code uses part of the code that was released into the public domain by
...
/* Named references to sub-sections of the Random64 array. */
static const uint64_t *piece_section = Random64;
// Offset by the number of squares times the number of different pieces.
static const uint64_t *castling_section = Random64 + (NUM_SQUARES * NUM_PIECES);
// Offset from castling_section by the number of different castling moves.
static const uint64_t *en_passant_section = Random64 + (NUM_SQUARES * NUM_PIECES) + 4;
// Offset from the en_passant_section by the number of en-passant columns.
static const uint64_t *white_to_move_element = Random64 + (NUM_SQUARES * NUM_PIECES) + 4 + 8;
...
/* Generate a Zobrist hash value from the FEN string passed as argument.
 * This has the form:
 ...
 */
uint64_t
generate_zobrist_hash_from_fen(const char *fen)
{
    Rank rank = LASTRANK;
    Col col = FIRSTCOL;
    ...
    while (Ok && (*fen_char != ' ') && (*fen_char != '\0') &&
        ...
        if (is_FEN_piece(ch)) {
	    board[rank - FIRSTRANK][col - FIRSTCOL] = ch;
	    hash ^= piece_hash(ch, rank, col);
            if (col <= LASTCOL) {
                col++;
                fen_char++;
            }
    ...
    if (*fen_char == '-') {
	    ...
	    fprintf(GlobalState.logfile, 
                    "Chess960 castling notation not supported in generating a hashcode.\n");
    ...
    if (*fen_char == '-') {
	    ...
	    if(to_move == 'b') {
	        if(col > FIRSTCOL && board['4' - FIRSTRANK][col - FIRSTCOL - 1] == 'p') {
		    usable = TRUE;
		}
		else if(col < LASTCOL && board['4' - FIRSTRANK][col - FIRSTCOL + 1] == 'p') {
		    usable = TRUE;
		}
	    }
	    else if(to_move == 'w') {
	        if(col > FIRSTCOL && board['5' - FIRSTRANK][col - FIRSTCOL - 1] == 'P') {
		    usable = TRUE;
		}
		else if(col < LASTCOL && board['5' - FIRSTRANK][col - FIRSTCOL + 1] == 'P') {
		    usable = TRUE;
		}
	    }
	    if(usable) {
		hash ^= en_passant_section[col - FIRSTCOL];
	    }
            fen_char++;
        }
        else {
...
#if 0
    /* The hash is now complete. The remainder of the function
     * just completes the checks for legality.
     */
...
/* Generate a Zobrist hash value from the Board passed as argument. */
uint64_t
generate_zobrist_hash_from_board(const Board *board)
{
    uint64_t hash = 0;

    /* The board. */
    /* Attempt to iterate over the board as fast as possible
     * as it is the main time consumer for this function.
     */
    int board_rank_index = RankConvert(FIRSTRANK);
    for (int r = 0; r < BOARDSIZE; r++, board_rank_index++) {
        int board_col_index = ColConvert(FIRSTCOL);
        for (int c = 0; c < BOARDSIZE; c++, board_col_index++) {
            int piece_id = -1;
            switch((int) board->board[board_rank_index][board_col_index]) {
                case B(PAWN):
                    piece_id = 0; break;
                case W(PAWN):
                    piece_id = 1; break;
            ...
            if (piece_id >= 0) {
		hash ^= piece_section[64 * piece_id + (8 * r) + c];
            }
        }
    }
    ...
    /* Castling details. */
    /* Chess960 requirements not yet dealt with. */
